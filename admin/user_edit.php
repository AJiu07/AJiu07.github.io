...
// 查询信息
$sql = "SELECT id,name,tel,email FROM user
WHERE id = '$id'";
$res = $pdo->query($sql);
...
if(!empty($_POST['tel'])){
	...
	$sql = "UPDATE user SET tel = '$tel',email = '$email' 
			WHERE id = '$id'";
	$res = $pdo->exec($sql);
	if($res){
								...
								<div class="row">
									<div class="col-md-12">
										<div class="form-group">
											...
											<input type="text" name="tel" value="<?php echo $k['tel']; ?>" class="form-control">
										</div>
									</div>
								</div>
								<div class="row">
									<div class="col-md-12">
										<div class="form-group">
											...
											<input type="email" name="email" value="<?php echo $k['email']; ?>" class="form-control">
										</div>
									</div>
								</div>						
